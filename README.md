...
- **Dashboard UI** — Liquid glass effects, model/key management, real-time account stats
...
> **Important:** Step Plan subscriptions use a dedicated endpoint `https://api.stepfun.ai/step_plan/v1` for chat completions. The proxy routes chat requests to this endpoint automatically. Using the standard `v1` endpoint with a Step Plan subscription will return `quota_exceeded` errors.
...
- **Account Stats** — Balance, Cash Balance, Voucher Balance (auto-refreshes every 60s)
...
├── HTTP Handlers         — OpenAI + management + account endpoints
...
No external npm dependencies — uses Node.js built-in modules only: `fs`, `path`, `os`, `http`, `https`, `url`, `crypto`.
